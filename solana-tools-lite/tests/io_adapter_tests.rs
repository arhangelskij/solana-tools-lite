...
use solana_tools_lite::adapters::io_adapter::{read_mnemonic, read_passphrase};
use solana_tools_lite::adapters::io_adapter::read_text_source;
use solana_tools_lite::adapters::io_adapter::{
    read_input_transaction, read_secret_key_file, save_pretty_json, write_public_file,
    write_secret_file, write_signed_transaction,
};
...
// save_pretty_json returns Ok(None) when out_path is None
#[test]
fn test_save_pretty_json_none_output_is_noop() -> Result<()> {
    let value = serde_json::json!({"ok": true});
    let saved = save_pretty_json(&value, None, false, "unused.json")?;
...
fn test_parse_signing_key_content_invalid_base58() {
    ...
    assert!(matches!(err, SignError::InvalidBase58));
}
...
fn test_parse_signing_key_content_invalid_length_array() {
    ...
    assert!(matches!(err, SignError::InvalidKeyLength));
}
...
fn test_write_output_to_file_adapter() -> Result<()> {
    ...
    let tx = solana_tools_lite::models::transaction::Transaction::try_from(ui).expect("ui -> tx");
    ...
    let tx2 =
        solana_tools_lite::models::transaction::Transaction::try_from(ui2).expect("ui2 -> tx");
...
// Write secret file: reject stdout and handle existing file without force
#[test]
fn test_write_secret_file_rejects_stdout_and_existing() -> Result<()> {
    let err = write_secret_file(std::path::Path::new("-"), "secret", false).unwrap_err();
    match err {
        ToolError::Io(IoError::IoWithPath { path: Some(p), .. }) => assert_eq!(p, "-"),
        other => panic!("Expected IoWithPath for stdout, got {other:?}"),
    }

    let path = "test_secret_out.txt";
    write_secret_file(std::path::Path::new(path), "secret", false)?;
    let err = write_secret_file(std::path::Path::new(path), "secret2", false).unwrap_err();
    match err {
        ToolError::FileExists { path: p } => assert!(p.ends_with(path)),
...
// Write public file: error when file exists and force is false
#[test]
fn test_write_public_file_existing_fails_without_force() -> Result<()> {
    let path = "test_public_out.txt";
    write_public_file(std::path::Path::new(path), "public", false)?;

    let err = write_public_file(std::path::Path::new(path), "public2", false).unwrap_err();
    match err {
        ToolError::Io(IoError::IoWithPath { path: Some(p), .. }) => assert!(p.ends_with(path)),
        other => panic!("Expected IoWithPath, got {other:?}"),
    }
...
// Read passphrase from file and trim trailing newlines only
#[test]
fn test_read_passphrase_trims_newlines() -> Result<()> {
    let path = "test_passphrase.txt";
    fs::write(path, "hunter2\r\n").map_err(|e| ToolError::Io(IoError::Io(e)))?;

    let pass = read_passphrase(path)?;
...
// Adapter: write_signed_transaction(Base64/Base58) writes wire bytes
#[test]
fn test_write_signed_transaction_base64_and_base58() -> Result<()> {
    let ui = test_utils::generate_ui_transaction(
        1,
        vec![
            "11111111111111111111111111111111",
    ...
    let path_b64 = "test_write_adapter.b64";
    write_signed_transaction(&tx, OutputFormat::Base64, Some(path_b64), false)?;
    let content_b64 = fs::read_to_string(path_b64).map_err(|e| ToolError::Io(IoError::Io(e)))?;
    assert_eq!(content_b64, BASE64.encode(&raw));
    fs::remove_file(path_b64).map_err(|e| ToolError::Io(IoError::Io(e)))?;

    let path_b58 = "test_write_adapter.b58";
    write_signed_transaction(&tx, OutputFormat::Base58, Some(path_b58), false)?;
    let content_b58 = fs::read_to_string(path_b58).map_err(|e| ToolError::Io(IoError::Io(e)))?;
    assert_eq!(content_b58, bs58::encode(&raw).into_string());
    fs::remove_file(path_b58).map_err(|e| ToolError::Io(IoError::Io(e)))?;
