mod tests_sign_tx {
    ...
    use solana_tools_lite::handlers::sign_tx::{handle, sign_transaction_by_key};
    ...
    use solana_tools_lite::codec::{
        deserialize_transaction, serialize_message, serialize_transaction,
    };
    ...
    // Additional parsing/roundtrip tests.
    #[test]
    ...
    // End-to-end sign -> serialize -> parse -> verify roundtrip.
    #[test]
    fn test_end_to_end_sign_and_save() {
        ...
        let msg_bytes = serialize_message(&tx2.message);
    ...
    /// New: handler-level test for handle success path
    #[test]
    fn test_handle_success() {
        ...
        // Call pure handler
        let result = handle(input_tx, &keypair).expect("handler should sign successfully");
    ...
    #[test]
    fn test_handle_signer_missing_fails() {
        ...
        let err = handle(input_tx, &keypair).unwrap_err();
        let msg = err.to_string();
        // Core error message is technical: "sign: SignerKeyNotFound" or "sign: SigningNotRequired"
        assert!(
            msg.contains("SignerKeyNotFound") || msg.contains("SigningNotRequired"),
            "unexpected error: {}",
            msg
        );
    }
    ...
    fn test_signature_invalid_on_message_tamper() {
        ...
        tampered_msg.account_keys_mut()[0] = generate_fake_pubkey();
    ...
    #[test]
    fn test_parse_base64_and_sign_roundtrip() {
        let seed = [7u8; 32];
        let keypair = ed25519::keypair_from_seed(&seed).unwrap();
        let verifying_key = keypair.verifying_key();

        // Build an unsigned transaction and convert it to Base64 wire bytes.
        let pk = bs58::encode(verifying_key.to_bytes()).into_string();
        let blockhash = generate_mock_pubkey();
        let unsigned_input = generate_input_transaction(
            1,
            vec![&pk, "11111111111111111111111111111111"],
            &blockhash,
            1,
            vec![0],
            "3Bxs4SHffsqLHuC3",
        );
        let unsigned_tx = Transaction::try_from(unsigned_input).unwrap();
        let base64_tx = BASE64.encode(&serialize_transaction(&unsigned_tx));

        let input = parse_input_transaction(Some(&base64_tx)).expect("detect format");
        let signed = handle(input, &keypair).expect("sign");

        let raw = serialize_transaction(&signed.signed_tx);
        let parsed = deserialize_transaction(&raw).expect("roundtrip deserialize");

        assert_eq!(parsed.signatures.len(), 1);
        let sig = parsed.signatures[0];
        let msg_bytes = serialize_message(&parsed.message);
        let ok = ed25519::verify_signature(&verifying_key, &msg_bytes, &sig);
        assert!(ok, "signature must verify after roundtrip");
    }
}
...
mod tests_signing {
    use super::common::{generate_input_transaction, generate_mock_pubkey};
