...
mod common;
use common::{generate_mock_pubkey, generate_mock_signature};
...
fn test_adapter_invalid_pubkey() {
    ...
    let result: std::result::Result<Transaction, _> = TryFrom::try_from(parsed);
...
fn test_adapter_invalid_base64_input() {
    ...
    let result: std::result::Result<Transaction, _> = TryFrom::try_from(invalid_base64);
...
fn test_adapter_invalid_base58_input() {
    ...
    let result: std::result::Result<Transaction, _> = TryFrom::try_from(invalid_base58);
    ...
    assert!(
        err.to_string()
            .to_lowercase()
            .contains("provided string contained invalid character '@' at byte 3"),
        "Expected base58 error, got: {err}"
    );
}

#[test]
fn test_adapter_base58_decodes_but_json_invalid() {
    let not_json = "definitely not json";
    let encoded = bs58::encode(not_json.as_bytes()).into_string();
    let input = InputTransaction::Base58(encoded);

    let result: std::result::Result<Transaction, _> = TryFrom::try_from(input);
...
#[test]
fn test_adapter_valid_base64_json() -> std::result::Result<(), Box<dyn std::error::Error>> {
    // This Base64 encodes a UiTransaction JSON with one signature, three account keys, and one instruction.
    let b64 = "";
...
#[test]
fn test_adapter_valid_base58_json() -> std::result::Result<(), Box<dyn std::error::Error>> {
    // This Base64 encodes a UiTransaction JSON with one signature, three account keys, and one instruction.
