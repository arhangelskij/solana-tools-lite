...
use solana_tools_lite::serde::input_tx::{
    is_base58 as is_b58, is_base64 as is_b64, parse_input_transaction,
};
...
fn test_parse_input_transaction_json_ok() {
    ...
    assert!(matches!(v, InputTransaction::Json(_)));
}
...
fn test_parse_input_transaction_base64_ok() {
    ...
    assert!(matches!(v, InputTransaction::Base64(_)));
}
...
fn test_parse_input_transaction_base58_ok() {
    ...
    assert!(matches!(v, InputTransaction::Base58(_)));
}
...
fn test_parse_input_transaction_invalid_format() {
    ...
    assert!(matches!(
        err,
        solana_tools_lite::errors::TransactionParseError::InvalidFormat(_)
    ));
}
