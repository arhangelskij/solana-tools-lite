...
pub fn handle_verify(message: &str, signature_b58: &str, pubkey_b58: &str) -> Result<()> {
    ...
    if sig_bytes.len() != SIG_LEN {
        bail!("Signature must be exactly {} bytes, got {}", SIG_LEN, sig_bytes.len());
    }

    // Ensure the public key is exactly PUBKEY_LEN bytes
    if pubkey_bytes.len() != PUBKEY_LEN {
        bail!("Public key must be exactly {} bytes, got {}", PUBKEY_LEN, pubkey_bytes.len());
    }
