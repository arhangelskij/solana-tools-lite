...
pub fn parse_input_transaction(input: Option<&str>) -> Result<InputTransaction, TransactionParseError> {
    match input {
...
fn parse_input_transaction_str(s: &str) -> Result<InputTransaction, TransactionParseError> {
    ...
    Err(TransactionParseError::InvalidFormat("Unknown input format".into()))
}
